...
export const requestOtp = createAsyncThunk<
  ForgetResponse, // success response type
  forgetCredentials, // request body type
...
>('auth/forget', async (userCredentials, { rejectWithValue }) => {
  try {
    ...
    // ✅ Extract error message safely from backend
    const errorMessage =
      error?.response?.data?.message ||
      error?.response?.data?.errorSources?.[0]?.message ||
      'Something went wrong. Please try again.';

    // Pass backend message to rejected case
...
export const { resetError, updateAuthIsCompleted, updateAuthIsValided } =
  authSlice.actions;
