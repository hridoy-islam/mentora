...
export default function InstructorPage() {
  ...
  const handleEdit = (instructor: any) => {
    ...
    // Reset form with instructor details, password empty
    form.reset({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const response = await axiosInstance.patch(`/users/${editingInstructor._id}`, payload);

      // Update local state
      setInstructors((prev) =>
        prev.map((item) =>
          item._id === editingInstructor._id ? { ...item, ...payload } : item
        )
      );
                  ...
                  <TableRow key={instructor._id}>
                    ...
                    <TableCell className="text-center">
                      <div className="flex flex-row items-center justify-center gap-2">
                        ...
                        >
                          {instructor.status === 'active' ? 'Active' : 'Disable'}
                        </span>
                      </div>
                    </TableCell>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="email@example.com" type="email" disabled {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
