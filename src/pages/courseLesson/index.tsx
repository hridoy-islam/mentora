...
export default function LessonsPage() {
  ...
  const handleSearch = () => fetchLessons(currentPage, entriesPerPage, searchTerm);
 ...
 const DraggableRow = ({ lesson, index }: any) => {
  ...
  return (
    <TableRow ref={ref} style={{ opacity: isDragging ? 0.4 : 1, cursor: "grab" }}>
      ...
      <TableCell>
        <Badge>{lesson.type.charAt(0).toUpperCase() + lesson.type.slice(1)}</Badge>
      </TableCell>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. Lesson "<b>{lesson.title}</b>" will be deleted permanently.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center space-x-4 mt-2">
              <Input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search lesson..." className="h-8 w-[200px]" />
              <Button onClick={handleSearch} className="h-8 bg-supperagent hover:bg-supperagent/90">
                <Search className="mr-2 h-4 w-4" /> Search
              </Button>
            </div>
            </div>
            {moduleName && <p className="mt-2 text-sm text-gray-600">Module: <b>{moduleName}</b></p>}
          </div>
          ...
          <div className="flex gap-4">
            ...
            <Button className="bg-supperagent hover:bg-supperagent/90" onClick={() => navigate(`create`)} disabled={!mid}>
        ...
        <Card>
          <CardContent className="pt-4">
            {loading ? (
              <div className="flex justify-center py-6"><BlinkingDots size="large" color="bg-supperagent" /></div>
            ) : lessons.length === 0 ? (
              <div className="flex flex-col items-center py-12 text-gray-500">
                <FileText className="h-12 w-12 mb-4 text-gray-400" />
                <p>No lessons found.</p>
                <Button className="mt-4 bg-supperagent hover:bg-supperagent/90" onClick={() => navigate(`create?module=${moduleId}&course=${courseId}`)}>
